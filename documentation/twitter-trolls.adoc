...
[source,cypher]
----
MATCH (u:Troll) WHERE u.followers_count IS NOT NULL
...
image::{img}/exampletweet1.png[]

== Tweet Volume 

image::{img}/volume.png" style="max-height: 380px;[]
...
image::{img}/amplified.png[]

== Graph Data Science Algorithms

link:https://neo4j.com/docs/graph-data-science/current/[Graph data science algorithms^] are a way to apply analytics to the entire graph to further enhance our understanding of the data. These algorithms fall into five major categories:

* **Centrality** - What are the most important nodes in the graph? _PageRank, Betweenness Centrality, Closeness Centrality_
* **Community detection** - How can the graph be clustered or partitioned? _Louvain, Leiden, Label Propagation, Connected Components_
* **Pathfinding** - What are the shortest paths or best routes available given cost? _Dijkstra, A* Shortest Path, Yen's Shortest Path, Minimum Weight Spanning Tree, Breadth-First and Depth-First Search_
* **Similarity** - Which nodes are similar in the graph? _Node Similarity, K-Nearest Neighbor (KNN)_
* **Embeddings** - How can we produce feature vectors based on graph structure for use in downstream analytics and ML? _FastRP, GraphSAGE, Node2vec, HashGNN_

We can run these algorithms in Neo4j with Cypher using the link:https://github.com/neo4j/graph-data-science[Graph Data Science^] procedures.

.See what procedures are available in the Graph Data Science library
[source,cypher]
----
CALL gds.list()
----
...
[source,cypher]
----
MATCH (t:Troll) WHERE t.pagerank IS NOT NULL
...
.Partition the graph into communities using the Label Propagation algorithm. An additional property `community` is added to each node specifying the community it has been assigned to by the algorithm.
[source,cypher]
----
